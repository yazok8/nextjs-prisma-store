...
export default function CartItem({item}:CartItemProps) {
    ...
    const formatedTotalCurrency = formatCurrency(totalPrice)

  return (
    <div className='grid grid-cols-5 text-xs md:text-sm gap-4 border-[1.5.px] border-slate-200 py-4 items-center'>
        ...
        <div className='flex flex-col justify-between'>
            ...
            <div className='w-[70px'>
                <button className='text-slate-500 underline' onClick={()=>{handleRemoveCartProduct(item)}}>Remove</button>
            </div>
        </div>
        </div>
